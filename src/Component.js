...
export class ScrollRotate extends React.Component {
  ...
  updateValues(e) {
    ...
    const topPercentage =
      (scrollTop /
        ((target ? se.scrollHeight : se.offsetHeight) - clientHeight)) *
      100;
  ...
  setScrollHandle() {
    let handleScrollThrottle = throttle(
      ...
      this.props.throttle ? 1000 * this.props.throttle : 0,
      { leading: false }
    );
  ...
  render() {
    ...
    return (
      ...
      >
        {getOnce && (
          <div style={{ ...wrapperStyles, ...rotateStyles }}>
            {this.props.children}
          </div>
        )}
      </div>
